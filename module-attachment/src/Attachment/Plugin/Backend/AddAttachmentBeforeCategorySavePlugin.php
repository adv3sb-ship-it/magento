 ...
 */
class AddAttachmentBeforeCategorySavePlugin
{
    ...
    public function beforeExecute(\Magento\Catalog\Controller\Adminhtml\Category\Save $subject)
    {
        ...
        $categoryId = (int)$data['entity_id'];
        if (!$categoryId) {
            return;
        }

        $attachmentsData = $this->getAttachmentsData($data);
        $this->attachmentService->saveForEntity(LinkInterface::ENTITY_TYPE_CATEGORY, $categoryId, $attachmentsData);
    }
